...
| 2026-06-05 01:00 | Project 6 — Security Pipeline (SecureBank) | Done | Go banking transaction API with shift-left security. 50 files, 7 stages, 17 exercises. 6/15 projects done. | projects/06-security-pipeline/, docs/data/labs/security-pipeline.json, docs/js/app.js:1003 |
| 2026-06-05 01:30 | Fix lab exercise links — intercept relative .md links | Done | Exercise/next-stage links inside rendered lab READMEs were 404ing. Now intercepted via click handler, resolved from stage dir, rendered in-app. | docs/js/app.js:1198-1235 |
| 2026-06-05 02:30 | Frontends for P2 + P3, P2 tests fixed, P3 Dockerfiles fixed | Done | P2 FinStack: payment dashboard (localhost:3000), 7/7 tests pass, Makefile/.gitignore/.env.example added. P3 ObservaShop: e-commerce dashboard (localhost:8000), fixed Dockerfiles (requirements.txt path) + port conflict (tempo/otel). Both frontends: dark theme, 5 files each (HTML/CSS/JS/Dockerfile/nginx), nginx proxy to API. All verified running. | projects/02-iac-pipeline/finstack/frontend/, projects/03-observability-stack/ObservaShop/frontend/ |
